...
def parse_arguments():
    ...
    args.add_argument('--bam', required=True,
                      help="The name of the Illumina BAM file for which to call structural variants containing mapped reads.")
    args.add_argument('--bai', required=False,
                      help="(Optional) The name of the corresponding index for the Illumina BAM file.")
    args.add_argument('-r', '--ref_genome', required=True,
                      help="The name of the reference file that matches the reference used to map the Illumina inputs.")
    args.add_argument('--fai', required=False,
                      help="(Optional) The name of the corresponding index for the reference genome file.")
    args.add_argument('--prefix', required=False,
                      help="(Optional) If provided, all output files will start with this. If absent, the base of the BAM file name will be used.")
    args.add_argument('--filter_short_contigs', action="store_true",
                      help="If selected, SV calls will not be generated on contigs shorter than 1 MB.")
    args.add_argument('--breakdancer', action="store_true",
                      help="If selected, the program Breakdancer will be one of the SV callers run.")
    args.add_argument('--breakseq', action="store_true",
                      help="If selected, the program BreakSeq2 will be one of the SV callers run.")
    args.add_argument('--manta', action="store_true",
                      help="If selected, the program Manta will be one of the SV callers run.")
    args.add_argument('--cnvnator', action="store_true",
                      help="If selected, the program CNVnator will be one of the SV callers run.")
    args.add_argument('--lumpy', action="store_true",
                      help="If selected, the program Lumpy will be one of the SV callers run.")
    args.add_argument('--delly_deletion', action="store_true",
                      help="If selected, the deletion module of the program Delly2 will be one of the SV callers run.")
    args.add_argument('--delly_insertion', action="store_true",
                      help="If selected, the insertion module of the program Delly2 will be one of the SV callers run.")
    args.add_argument('--delly_inversion', action="store_true",
                      help="If selected, the inversion module of the program Delly2 will be one of the SV callers run.")
    args.add_argument('--delly_duplication', action="store_true",
                      help="If selected, the duplication module of the program Delly2 will be one of the SV callers run.")
    args.add_argument('--genotype', action="store_true",
    ...
    args.add_argument('--svviz', action="store_true",
                      help="If selected, visualizations of genotyped SV events will be produced with SVVIZ, one screenshot of support per event. For this option to take effect, Genotype must be selected.")
    args.add_argument('--svviz_only_validated_candidates', action="store_true",
                      help="Run SVVIZ only on validated candidates? For this option to be relevant, SVVIZ must be selected. NOT selecting this will make the SVVIZ component run longer.")
...
def run_parliament(bam, bai, ref_genome, fai, prefix, filter_short_contigs, breakdancer, breakseq, manta, cnvnator,
    ...
    if bai is not None:
        if fai is not None:
            ...
            subprocess.check_call(
                ['bash', 'parliament2.sh', bam, bai, ref_genome, "None", prefix, str(filter_short_contigs),
                 str(breakdancer), str(breakseq), str(manta), str(cnvnator), str(lumpy), str(delly_deletion),
                 str(delly_insertion), str(delly_inversion), str(delly_duplication), str(genotype), str(svviz),
                 str(svviz_only_validated_candidates), str(dnanexus)])
    else:
        if fai is not None:
            ...
            subprocess.check_call(
                ['bash', 'parliament2.sh', bam, "None", ref_genome, "None", prefix, str(filter_short_contigs),
                 str(breakdancer), str(breakseq), str(manta), str(cnvnator), str(lumpy), str(delly_deletion),
                 str(delly_insertion), str(delly_inversion), str(delly_duplication), str(genotype), str(svviz),
                 str(svviz_only_validated_candidates), str(dnanexus)])
