...
ref_genome=$(python /home/dnanexus/get_reference.py "${ref_fasta}")
...
if [[ "${run_breakseq}" == "True" ]]; then
    ...
    work="breakseq2"
    timeout 6h /home/dnanexus/breakseq2-2.2/scripts/run_breakseq2.py --reference "${ref_fasta}" \
        --bams "${illumina_bam}" --work "${work}" \
...
if [[ "${run_breakdancer}" == "True" ]]; then
    timeout 2h /breakdancer/cpp/bam2cfg -o breakdancer.cfg "${illumina_bam}"
fi
        ...
        if [[ $(samtools view "${illumina_bam}" "${contig}" 2>/dev/null | head -n 20 | wc -l) -ge 10 ]]; then
            ...
            if [[ "${run_delly}" == "True" ]] || [[ "${run_lumpy}" == "True" ]]; then
                ...
                if [[ "${run_delly_deletion}" == "True" ]]; then
                    ...
                    timeout 6h delly -t DEL -o "${count}".delly.deletion.vcf -g "${ref_fasta}" chr."${count}".bam 1> /home/dnanexus/out/log_files/delly_deletion_logs/"${prefix}".delly_deletion."${contig}".stdout.log 2> /home/dnanexus/out/log_files/delly_deletion_logs/"${prefix}".delly_deletion."${contig}".stderr.log &
                ...
                if [[ "${run_delly_inversion}" == "True" ]]; then
                    ...
                    timeout 6h delly -t INV -o $count.delly.inversion.vcf -g "${ref_fasta}" chr."${count}".bam 1> /home/dnanexus/out/log_files/delly_inversion_logs/"${prefix}".delly_inversion."${contig}".stdout.log 2> /home/dnanexus/out/log_files/delly_inversion_logs/"${prefix}".delly_inversion."${contig}".stderr.log &
                ...
                if [[ "${run_delly_duplication}" == "True" ]]; then
                    ...
                    timeout 6h delly -t DUP -o "${count}".delly.duplication.vcf -g "${ref_fasta}" chr."${count}".bam 1> /home/dnanexus/out/log_files/delly_duplication_logs/"${prefix}".delly_duplication.stdout.log 2> /home/dnanexus/out/log_files/delly_duplication_logs/"${prefix}".delly_duplication.stderr.log &
                ...
                if [[ "${run_delly_insertion}" == "True" ]]; then
                    ...
                    timeout 6h delly -t INS -o "${count}".delly.insertion.vcf -g "${ref_fasta}" chr."${count}".bam 1> /home/dnanexus/out/log_files/delly_insertion_logs/"${prefix}".delly_insertion."${count}".stdout.log 2> /home/dnanexus/out/log_files/delly_insertion_logs/"${prefix}".delly_insertion."${count}".stderr.log &
...
if [[ "${run_genotype_candidates}" == "True" ]]; then
    ...
    if [[ "${run_breakdancer}" == "True" ]]; then
        ...
        if [[ -f breakdancer.vcf ]]; then
            bash /home/dnanexus/parallelize_svtyper.sh breakdancer.vcf svtype_breakdancer "${prefix}".breakdancer.svtyped.vcf "${illumina_bam}"
    ...
    if [[ "${run_breakseq}" == "True" ]]; then
        ...
        if [[ -f breakseq.vcf ]]; then
            bash /home/dnanexus/parallelize_svtyper.sh breakseq.vcf svtype_breakseq "${prefix}".breakseq.svtyped.vcf "${illumina_bam}"
        else
    ...
    if [[ "${run_cnvnator}" == "True" ]]; then
        ...
        if [[ -f cnvnator.vcf ]]; then
            ...
            bash /home/dnanexus/parallelize_svtyper.sh cnvnator.vcf svtype_cnvnator "${prefix}".cnvnator.svtyped.vcf "${illumina_bam}"
        else
    ...
    if [[ "${run_delly}" == "True" ]]; then
        ...
        if [[ -z $(find . -name "delly*vcf") ]]; then
            ...
            for item in delly*vcf; do
                ...
                bash /home/dnanexus/parallelize_svtyper.sh "${item}" svtype_delly_"${i}" delly.svtyper."${i}".vcf "${illumina_bam}"
                i=$((i + 1))
            done
    ...
    if [[ "${run_svviz}" == "True" ]]; then
        ...
        if [[ "${vcf_entries}" == 0 ]]; then
            ...
            for item in small_vcf*; do
                ...
                echo "svviz --pair-min-mapq 30 --max-deletion-size 5000 --max-reads 10000 --fast --type batch --summary svviz_summary.tsv -b \"${illumina_bam}\" \"${ref_fasta}\" survivor_split.${count}.vcf --export svviz_outputs" >> commands.txt
                ((count++))
            done
